...
<form method="POST" action="{{ isset($task) ? route('tasks.update', ['task' => $task->id]) : route('tasks.store') }}">
  ...
  <div class="mb-4">
    ...
    value="{{ (!isset($task) || $errors->any()) ? old('title') : $task->title }}" />
    @error('title')
  ...
  <div class="mb-4">
    ...
    <textarea name="description" id="description" @class(['border-red-500'=> $errors->has('description')])
      rows="5">{{ (!isset($task) || $errors->any()) ? old('description') : $task->description }}</textarea>
    @error('description')
    <p class="error">{{ $message }}</p>
    @enderror
  </div>
  ...
  <div class="mb-4">
    ...
    <textarea name="long_description" id="long_description" @class(['border-red-500'=> $errors->has('long_description')])
      rows="10">{{ (!isset($task) || $errors->any()) ? old('long_description') : $task->long_description }}</textarea>
